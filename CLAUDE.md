...
# Force a USD→BRL exchange rate sync on demand (admin JWT required; runs on boot/daily anyway)
curl -X POST "http://localhost:5052/api/admin/sync-exchange-rate" -H "Authorization: Bearer <admin-jwt>"
...
- **Infrastructure** — EF Core 9 (Npgsql/PostgreSQL) `MtgDeckLabDbContext`, entity configurations, migrations, repository implementations, JWT/password-hashing implementations, and the Scryfall integration (`ScryfallSyncService` streams the Scryfall bulk-data JSON and upserts the `cards` table; `ScryfallSyncBackgroundService` runs it on a schedule — configurable via `Scryfall:ScheduledSyncEnabled`/`Scryfall:SyncIntervalHours`; `ScryfallTranslationSyncBackgroundService` does the same for translated card names under `Scryfall:Translations:*`). The translation catalogues (`Resources/Localization/*.resx`) and their localizers live here as well. Same shape for currency: `ExchangeRateSyncBackgroundService` fetches the USD→BRL rate daily from [Frankfurter](https://api.frankfurter.dev) (free, no API key) into an in-memory `IExchangeRateStore` — configurable via `ExchangeRates:ScheduledSyncEnabled`/`ExchangeRates:SyncIntervalHours` — but unlike the Scryfall services it also fetches once immediately at startup (a single lightweight HTTP call, not worth waiting a day for) rather than only on the schedule. `GET /api/exchange-rate` exposes the cached rate; the frontend (not the API) does the actual USD→BRL conversion, showing R$ only when the UI language is pt-BR (`currency/CurrencyContext.tsx`) — prices themselves stay canonical in USD everywhere in the backend.
...
Card prices are USD everywhere in the API; the frontend converts to BRL for display only, via `useCurrency()`/`CurrencyProvider` in `currency/CurrencyContext.tsx` (fetches the cached rate from `GET /api/exchange-rate` once per session) — not through `i18n/format.ts`, which only handles number *notation*, not currency conversion.
...
- `src/currency/` — `CurrencyProvider`/`useCurrency()`, converts and formats card/deck USD prices as BRL when the UI language is pt-BR.
